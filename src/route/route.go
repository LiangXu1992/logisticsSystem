...
func InitRoute(c *gin.Engine) {
	//物流根目录
	c.POST(common.LOGISTICS_ROOT, controllers.LogisticsIndex)
	//物流员登录
	c.POST(common.LOGISTICS_ROOT+"login", databases.InitDb, controllers.LogisticsLogin, databases.CloseDb)
	//物流员订单列表
	c.POST(common.LOGISTICS_ROOT+"order", databases.InitDb, controllers.LogisticsOrder, databases.CloseDb)
